...
```
in various geometries of Stokes flow. The above code can be modified such that `unbounded` is replaced by `wallBounded`, `interface`, `periodic` etc. The instantiation can then be used to compute `flow` and `Rbm` due to each irreducible mode of the force per unit area on the surface of the particle. This is illustrate through fully coded examples on [GitHub](https://github.com/rajeshrinet/pystokes).  
