...
class linearSolve_direct:
    ...
    def get_F2s(self, r, F, T, S):
        ...
        GHH = np.zeros([6*Np,6*Np])   #there is only F2s, while V2s=0 -- keep option to retain V2s as system needs to be quadratic anyway
        KHH  = np.zeros([6*Np,6*Np])
        GH1s = np.zeros([6*Np,3*Np])
        GH2a = np.zeros([6*Np,3*Np])
        ...
        for i in range(Np):
            ...
            VH[6*i:6*i+6]   = np.array([S[i],S[i+Np],S[i+2*Np],S[i+3*Np],S[i+4*Np],S[i+5*Np]])
            ...
            for j in range(Np):
                if j!=i: ## off diagonals
                    ...
                    GHH[6*i:6*(i+1), 6*j:6*(j+1)] = me.G2s2s(xij,yij,zij, b,eta)
                    KHH[6*i:6*(i+1), 6*j:6*(j+1)] = me.K2s2s(xij,yij,zij, b,eta)
                    GH1s[6*i:6*(i+1), 3*j:3*(j+1)] = me.G2s1s(xij,yij,zij, b,eta)
                    GH2a[6*i:6*(i+1), 3*j:3*(j+1)] = me.G2s2a(xij,yij,zij, b,eta)
                    
                    
                else: ## fill diagonal elements of GHH etc, j==i
                    GHH[6*j:6*(j+1), 6*j:6*(j+1)] = me.Go2s2s(b, eta)
                    KHH[6*j:6*(j+1), 6*j:6*(j+1)] = - me.Ko2s2s(b, eta)
