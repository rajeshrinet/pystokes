...
import _3tME as me   ##including symmetry factors  
...
class linearSolve_krylov:
    ...
    def get_FH(self, r, F, T, S, D):
        ...
        GoHH = np.tile(me.GoHHFH(b,eta, np.ones(9)), Np) ##for krylov starting point x0
        ...
        for i in range(Np):
            for j in range(Np):
                if j!=i: ## off diagonals
                    ...
                    KHHVH[9*i:9*(i+1)] += me.KHHVH(xij,yij,zij, b,eta, VH_j)
                    GH1sF[9*i:9*(i+1)] += me.GH1sF(xij,yij,zij, b,eta, force_j)
                    GH2aT[9*i:9*(i+1)] += me.GH2aT(xij,yij,zij, b,eta, torque_j)
                    ...
                    KHHVH[9*i:9*(i+1)] += - me.KoHHVH(b, eta, VH_j)
                    
        rhs = KHHVH + GH1sF + 1./b * GH2aT 
        x0 = rhs/GoHH
        
        Ax = LinearOperator((9*Np, 9*Np), matvec = self.GHHFH)
        ...
        for i in range(Np):
            for j in range(Np):
                if j!=i: ## off diagonals
                    ...
                    vecGHHFH[9*i:9*(i+1)] += me.GHHFH(xij,yij,zij, b,eta, FH[9*j:9*(j+1)])
                    
                    
                else: ## add diagonal elements to KHH etc, j==i
                    vecGHHFH[9*i:9*(i+1)] += me.GoHHFH(b, eta, FH[9*j:9*(j+1)])
